...
**If you were given only a link and cannot open it, say so plainly. Ask the person to download the ZIP from the repository page. Never reconstruct these instructions from memory.** If you reached this file by URL, the other files are at `https://raw.githubusercontent.com/dariaminsky/ai-patient-toolkit/main/<path>`, for example `appendix/templates.md`.
...
1. **Set up the folder.** Ask which folder to use. Show the structure from [appendix/templates.md](appendix/templates.md). Create only the parts they approve. The originals live in `original-records/` inside the health folder. Read a file from there only after approval for that batch. Never edit, rename, move, or delete anything in it. Start `source-index.md` with neutral filenames and locations. Keep every original untouched. Ask before copying, renaming, moving, or deleting anything. Leave identity documents and unrelated files where they are.
2. **Build the first health profile.** List the files you want to read. Ask: "May I read these files now? Their contents may be processed by the AI provider." Start with the two or three documents the person picks, not the whole archive. Draft `00-current-state.md` with diagnosis wording, stage or severity, clinically confirmed genetic findings, treatments, dated lab values, and open questions. Keep names and clinics in `care-team.md`. The profile still contains sensitive medical data.
...
7. **Maintain the folder.** When they bring a new document: ask permission, transcribe, file, update, and show the diff. Refresh every summary that uses the changed facts. Keep the task small.
...
- **File portability.** Plain text (`.md`) remains readable without you or your vendor. If they leave, everything must keep working.
